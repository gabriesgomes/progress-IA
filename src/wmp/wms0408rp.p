...
PUT UNFORMATTED
    "Movimento; Item Documento; Seq; Status Movto; Tipo Movto;"
    "Qtd Movto; Endere‡o Dest.; Dep¢sito; Bloco; Rua;"
    "Coluna; N¡vel; Posi‡Æo;"
    SKIP .
...
IF tt-param.tipo_movimento = 0 THEN DO:
    FOR EACH wms_movimento NO-LOCK
        WHERE wms_movimento.id_movimento >= tt-param.id_movimento-ini
        AND wms_movimento.id_movimento <= tt-param.id_movimento-fim
        AND wms_movimento.id_item_documento >= tt-param.id_item_documento-ini
        AND wms_movimento.id_item_documento <= tt-param.id_item_documento-fim
        AND wms_movimento.id_endereco_destinado >= tt-param.id_endereco_destinado-ini
        AND wms_movimento.id_endereco_destinado <= tt-param.id_endereco_destinado-fim
        :
...
ELSE DO:
    FOR EACH wms_movimento NO-LOCK
        ...
        AND wms_movimento.id_endereco_destinado >= tt-param.id_endereco_destinado-ini
        AND wms_movimento.id_endereco_destinado <= tt-param.id_endereco_destinado-fim
        AND wms_movimento.tipo_movimento = tt-param.tipo_movimento
        :
        FIND FIRST wms_endereco NO-LOCK
            WHERE wms_endereco.id_endereco = wms_movimento.id_endereco_destinado
            .
...
PROCEDURE pi-monta-relatorio
    :
    CASE wms_movimento.status_movto_wms :
        WHEN 1 THEN
    ...
    CASE wms_movimento.tipo_movimento :
        WHEN 1 THEN
            ASSIGN cTipoMovto = "Entrada" . 
        WHEN 2 THEN
            ASSIGN cTipoMovto = "Sa¡da" . 
    END CASE.

    RUN pi-acompanhar IN h-acomp (INPUT 'Movimento: ' + STRING(wms_movimento.id_movimento)) .

    PUT UNFORMATTED
            STRING(wms_movimento.id_movimento, ">>,>>>,>>>")
        ';' STRING(wms_movimento.id_item_documento, ">>,>>>,>>>")
        ';' wms_movimento.sequencia
        ';' cStatusDoc
        ';' cTipoMovto
        ';' wms_movimento.qtde_movimento
        
        ';' wms_movimento.id_endereco_destinado
        ';' wms_endereco.cod_depos
        ';' wms_endereco.cod_bloc
