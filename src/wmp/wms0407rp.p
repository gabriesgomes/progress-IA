...
PUT UNFORMATTED
    "Item Documento; Documento; Dt Gera‡Æo; Tipo Documento;Seq;"
    "Item; Referˆncia; Lote; Status Item Doc.; Qtd. Item" 
    SKIP .
...
FOR EACH wms_item_documento NO-LOCK
    WHERE wms_item_documento.id_item_documento >= tt-param.id_item_documento-ini
    AND wms_item_documento.id_item_documento <= tt-param.id_item_documento-fim
    AND wms_item_documento.id_documento >= tt-param.id_documento-ini
    AND wms_item_documento.id_documento <= tt-param.id_documento-fim
    :
    ...
    CASE wms_item_documento.status_item_docto_wms :
        WHEN 1 THEN
    ...
    RUN pi-acompanhar IN h-acomp (INPUT 'Item Documento: ' + STRING(wms_item_documento.id_item_documento)) .

    PUT UNFORMATTED
            STRING(wms_item_documento.id_item_documento, ">>,>>>,>>>")
        ';' STRING(wms_item_documento.id_documento, ">>,>>>,>>>")
        ';' STRING(wms_documento.data_geracao, "99/99/9999")
        ';' cTipoDoc
        ';' wms_item_documento.sequencia

        ';' UPPER(wms_item_documento.cod_item)
        ';' wms_item_documento.referencia
        ';' wms_item_documento.lote
        ';' cStatusDoc
