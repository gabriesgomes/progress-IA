...
CREATE tt-filter . ASSIGN
    tt-filter.cod-estabel-ini   = ""
    tt-filter.cod-estabel-fim   = "ZZZZZ"
    tt-filter.serie-docto-ini   = ""
    tt-filter.serie-docto-fim   = "ZZZZZ"
    tt-filter.nro-docto-ini   = ""
    tt-filter.nro-docto-fim   = "ZZZZZZZZZZ"
...
&Scoped-define FIELDS-IN-QUERY-brTable tt_wms_tarefa_conferencia.id_tarefa_conferencia wms_documento.cod_estabel wms_documento.serie_docto wms_documento.nro_docto wms_item_documento.cod_item ITEM.desc-item tt_wms_tarefa_conferencia.qtde_tarefa   
...
btEtiquetas btQueryJoins btReportsJoins btExit btHelp brTable f-log-obs 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable wWindow _FREEFORM
  QUERY brTable DISPLAY
      tt_wms_tarefa_conferencia.id_tarefa_conferencia
      ...
      tt_wms_tarefa_conferencia.qtde_tarefa
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DO:
    ...
    IF tt_wms_tarefa_conferencia.status_tarefa_wms = 2 /* Liberada */ THEN DO:
        RUN wmp/wms0312b.w PERSISTENT SET h-son (
    ...
    IF tt_wms_tarefa_conferencia.status_tarefa_wms = 2 /* Liberada */ THEN DO:
        RUN wmp/wms0312c.w
            (INPUT THIS-PROCEDURE ,
             INPUT tt_wms_tarefa_conferencia.id_tarefa )
            .
    END.
    ELSE DO:
        RUN utp/ut-msgs.p
            (INPUT "Show":U , INPUT "17242" , INPUT
             "Conferància n∆o est† liberada!" + "~~" + 
             "A conferencia deste documento j† foi Realizada ou ainda n∆o foi Liberada." )
            .
    END.
END.
...
FOR EACH wms_tarefa_conferencia NO-LOCK
    ...
    AND wms_tarefa_conferencia.tipo_conferencia = 2 /* Expediá∆o */
    ,
    ...
    AND wms_documento.cod_estabel >= tt-filter.cod-estabel-ini  
    AND wms_documento.cod_estabel <= tt-filter.cod-estabel-fim 
    AND wms_documento.serie_docto >= tt-filter.serie-docto-ini 
    AND wms_documento.serie_docto <= tt-filter.serie-docto-fim 
    AND wms_documento.nro_docto >= tt-filter.nro-docto-ini 
    AND wms_documento.nro_docto <= tt-filter.nro-docto-fim 
    : 
