...
&SCOPED-DEFINE ProgramZoom          wmzoom/z01wms017.w
...
&SCOPED-DEFINE page1EnableWidgets   brTable1 brTable2 btAddPage1 btTarefas btGerarEtiquetas btDestinarAut btGerarEtiquetasEan
&SCOPED-DEFINE page1DisplayFields

&SCOPED-DEFINE page1DBOTable        wms_item_documento
&SCOPED-DEFINE page1SonProgram      wmp/wms0301a.w
&SCOPED-DEFINE page2SonProgram      wmp/wms0301b.w
&SCOPED-DEFINE page3SonProgram      wmp/wms0304.w
&SCOPED-DEFINE page4SonProgram      wmp/wms0402.w
&SCOPED-DEFINE page5SonProgram      wmp/wms0301rp.p
...
DEF NEW SHARED VAR iDoca   AS INT NO-UNDO .
...
DEF NEW SHARED VAR iIdDocumento AS INT NO-UNDO .
DEF VAR lEtiquetaEAN AS LOG INIT NO COLUMN-LABEL "Sel" NO-UNDO .
...
DEF VAR iQtdeEtiqueta AS DEC INIT 0.0 NO-UNDO.
DEF VAR iCountEtiqueta AS INT INIT 0  NO-UNDO.
...
&Scoped-define FIELDS-IN-QUERY-brTable1 lEtiquetaEan {&page1DBOTable}.cod_item item.desc-item {&page1DBOTable}.lote ENTRY({&page1DBOTable}.status_item_docto_wms,cStatusWMS,",") @ cStatusWMS {&page1DBOTable}.qtde_item {&page1DBOTable}.id_item_documento {&page1DBOTable}.sequencia   
...
tt_wms_documento.nat_operacao tt_wms_documento.cod_embarque ~
tt_wms_documento.identificador 
...
btSave btRefresh btLiberarConferencia btQueryJoins btReportsJoins btExit ~
btHelp cb-tipo-documento cb-status-docto-wms tg-avalia-cq ~
tg-armazena-especial 
...
tg-avalia-cq tg-armazena-especial 
...
DEFINE BUTTON btLiberarConferencia 
     IMAGE-UP FILE "image/im-f-ldo.bmp":U
     LABEL "Liberar Conferància" 
     SIZE 4 BY 1.25
     FONT 4.
...
DEFINE VARIABLE tg-armazena-especial AS LOGICAL INITIAL no 
     LABEL "Armazena Especial" 
     VIEW-AS TOGGLE-BOX
     SIZE 16 BY .71 NO-UNDO.

DEFINE VARIABLE tg-avalia-cq AS LOGICAL INITIAL no 
     LABEL "Avaliaá∆o CQ" 
     VIEW-AS TOGGLE-BOX
...
DEFINE BUTTON btGerarEtiquetas 
     LABEL "Gerar Etiquetas" 
     SIZE 12 BY 1.

DEFINE BUTTON btGerarEtiquetasEan 
     LABEL "Gerar Etiquetas EAN" 
     SIZE 16 BY 1.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS brTable1 wWindow _FREEFORM
  QUERY brTable1 DISPLAY
      lEtiquetaEan VIEW-AS TOGGLE-BOX
      ...
      {&page1DBOTable}.sequencia FORMAT ">>>>>99"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH SEPARATORS SIZE 105 BY 5
         FONT 1.
...
DEFINE FRAME fPage0
     ...
     btRefresh AT ROW 1.13 COL 67 HELP
          "Atualizar" WIDGET-ID 156
     btLiberarConferencia AT ROW 1.13 COL 85 HELP
     ...
     tt_wms_documento.cod_estabel AT ROW 5.5 COL 17 COLON-ALIGNED WIDGET-ID 86
          VIEW-AS FILL-IN 
          SIZE 4.72 BY .88
     tt_wms_documento.nro_docto AT ROW 5.5 COL 40 COLON-ALIGNED WIDGET-ID 94
          VIEW-AS FILL-IN 
          SIZE 17 BY .88
     tt_wms_documento.serie_docto AT ROW 5.5 COL 65 COLON-ALIGNED WIDGET-ID 96
          VIEW-AS FILL-IN 
          SIZE 6 BY .88
     tt_wms_documento.cod_emitente AT ROW 5.5 COL 91 COLON-ALIGNED WIDGET-ID 84
          VIEW-AS FILL-IN 
          SIZE 7.57 BY .79
     tg-avalia-cq AT ROW 6.42 COL 93 WIDGET-ID 158
     tt_wms_documento.nat_operacao AT ROW 6.5 COL 17 COLON-ALIGNED WIDGET-ID 92
          VIEW-AS FILL-IN 
          SIZE 7 BY .88
     tt_wms_documento.cod_embarque AT ROW 6.5 COL 40 COLON-ALIGNED WIDGET-ID 82
          VIEW-AS FILL-IN 
          SIZE 11 BY .88
     tt_wms_documento.identificador AT ROW 6.5 COL 65 COLON-ALIGNED WIDGET-ID 164
          VIEW-AS FILL-IN 
          SIZE 20 BY .88
     tg-armazena-especial AT ROW 7.08 COL 93 WIDGET-ID 162
...
DEFINE FRAME fPage1
     ...
     btGerarEtiquetas AT ROW 6.33 COL 48 WIDGET-ID 16
     btGerarEtiquetasEan AT ROW 6.33 COL 60 WIDGET-ID 18
...
ON CHOOSE OF btDestinar IN FRAME fPage1 /* Destinaá∆o Manual */
DO:
        ...
        IF wms_item_documento.status_item_docto_wms = 1 /* Pendente */ THEN DO:
            RUN utp/ut-msgs.p
                (INPUT "Show":U , INPUT "27100" , INPUT
                 "Deseja Gerar os movimentos de Entrada deste Item?" + "~~" + 
                 "Criar movimentos autom†ticos." )
                .
            IF RETURN-VALUE = "YES" THEN DO:
                RUN wmapi/wmsapi005.p(INPUT wms_item_documento.id_item_documento) .
            END.
        END.
        ELSE DO:
            RUN utp/ut-msgs.p
...
&Scoped-define FRAME-NAME fPage1
&Scoped-define SELF-NAME btGerarEtiquetas
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btGerarEtiquetas wWindow
ON CHOOSE OF btGerarEtiquetas IN FRAME fPage1 /* Gerar Etiquetas */
DO:
    ASSIGN 
        cItem = {&page1DBOTable}.cod_item 
        cLote = {&page1DBOTable}.lote 
        dQtde = {&page1DBOTable}.qtde_item
        cEstab = {&DBOTempTable}.cod_estabel
        cNrDocto = {&DBOTempTable}.nro_docto
        cSerieDocto = {&DBOTempTable}.serie_docto
        cCodEmitente = STRING({&DBOTempTable}.cod_emitente)
        cNatOperacao = {&DBOTempTable}.nat_operacao
        iIdDocumento = {&DBOTempTable}.id_documento
        .
...
&Scoped-define SELF-NAME btGerarEtiquetasEan
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btGerarEtiquetasEan wWindow
ON CHOOSE OF btGerarEtiquetasEan IN FRAME fPage1 /* Gerar Etiquetas EAN */
DO:
    EMPTY TEMP-TABLE tt-wms_item_documento.
    
    IF {&DBOTempTable}.status_docto_wms = 1  THEN DO:
        RUN utp/ut-msgs.p (INPUT "ERROR",
                           INPUT 17006,
                           INPUT "Documento est† pendente!" + "~~" +
                                 "N∆o Ç poss°vel gerar etiquetas de itens com documento pendente.").
        RETURN "NOK".
        
    END.
    
    IF CAN-FIND (FIRST wms_tarefa_conferencia
                 WHERE wms_tarefa_conferencia.id_item_documento       = {&page1DBOTable}.id_item_documento
                   AND CAN-FIND (FIRST wms_doca
                                 WHERE wms_doca.id_doca       = wms_tarefa_conferencia.id_doca 
                                   AND ((wms_doca.cod_estabel  = '108'  /*  Dep¢sito de insumos AFP */
                                   AND wms_doca.cod_depos     = '50') 
                                    OR (wms_doca.cod_estabel  = '111'  /*  Dep¢sito de insumos Linhares  */
                                   AND wms_doca.cod_depos     = '85' )) ) ) THEN DO:    
    END.
    ELSE DO:
        RUN utp/ut-msgs.p (INPUT "SHOW":U,
                           INPUT 17006,
                           INPUT "N∆o Ç permitido gerar etiquetas!" + "~~" +
                                 "Somente Ç poss°vel gerar etiquetas EAN para dep¢sito de insumos.").
        RETURN "NOK".
    END.
        
    FOR EACH {&page1DBOTable} NO-LOCK
       WHERE {&page1DBOTable}.id_documento = {&DBOTempTable}.id_documento:
        FIND FIRST wms_item_embalagem NO-LOCK
             WHERE wms_item_embalagem.cod_item = {&page1DBOTable}.cod_item NO-ERROR.
        IF AVAIL wms_item_embalagem THEN DO:
            ASSIGN iQtdeEtiqueta = {&page1DBOTable}.qtde_item / wms_item_embalagem.quantidade_agrup.
            
            IF iQtdeEtiqueta > INT(iQtdeEtiqueta) THEN DO:
                ASSIGN iQtdeEtiqueta =  iQtdeEtiqueta + 1.              
            END.
            
            DO iCountEtiqueta = 1 TO INT(iQtdeEtiqueta):
                
                CREATE tt-wms_item_documento.
                ASSIGN tt-wms_item_documento.id_documento      = {&page1DBOTable}.id_documento 
                       tt-wms_item_documento.id_item_documento = {&page1DBOTable}.id_item_documento
                       tt-wms_item_documento.cod_item          = {&page1DBOTable}.cod_item.
            END.            
        END.
        ELSE DO:
            CREATE tt-wms_item_documento.
        ...
        END.
    END.
    RUN {&page5SonProgram} (INPUT TABLE tt-wms_item_documento).
...
&Scoped-define SELF-NAME btLiberarConferencia
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btLiberarConferencia wWindow
ON CHOOSE OF btLiberarConferencia IN FRAME fPage0 /* Liberar Conferància */
DO:
    IF {&DBOTempTable}.status_docto_wms = 1 /* Pendente */ OR {&DBOTempTable}.status_docto_wms = 4 /* Conferencia */ THEN DO:
        RUN wmp/wms0301c.w
            (INPUT THIS-PROCEDURE ,
    ...
    ELSE DO:
        RUN utp/ut-msgs.p
            (INPUT "Show":U , INPUT "17242" , INPUT
             "Conferància j† foi Realizada!" + "~~" + 
             "A conferencia deste documento j† foi Realizada anteriormente." )
            .
    END.
...
DO WITH FRAME fPage0
    ...
    IF {&DBOTempTable}.tipo_documento = 0 THEN DO:
        ASSIGN cb-tipo-documento:SCREEN-VALUE = "1" /* Pendente */ .
    END.
    ELSE DO:
    ...
    IF {&DBOTempTable}.status_docto_wms = 0 THEN DO:
        ASSIGN cb-status-docto-wms:SCREEN-VALUE = "1" /* Pendente */ .
    END.
    ELSE DO:
    ...
    ASSIGN 
        tg-avalia-cq:CHECKED = NO 
        tg-armazena-especial:CHECKED = NO 
        .
    IF {&DBOTempTable}.avalia_cq = YES THEN DO: 
        ASSIGN tg-avalia-cq:CHECKED = YES .
    END.
    IF {&DBOTempTable}.armazena_especial = YES THEN DO: 
        ASSIGN tg-armazena-especial:CHECKED = YES.
    END.
END.

/*Carregar Browse e controlar seus botoes*/
DO WITH FRAME fPage0:
    ASSIGN btLiberarConferencia:SENSITIVE = YES .
END .
...
DO WITH FRAME fPage1:
    ...
    ELSE DO:
        ...
        IF brTable1:QUERY:NUM-RESULTS >= 1 THEN DO:
            ...
            //ASSIGN btDestinarAut:SENSITIVE = YES .
            //ASSIGN btDestinar:SENSITIVE = YES .
            //ASSIGN btGerarEtiquetas:SENSITIVE = YES .
        END.
        ELSE DO:
            ASSIGN btUpdatePage1:SENSITIVE = NO .
            ASSIGN btDeletePage1:SENSITIVE = NO .
            ...
            //ASSIGN btGerarEtiquetas:SENSITIVE = NO .
        END.
        IF brTable2:QUERY:NUM-RESULTS >= 1 THEN DO:
            ASSIGN btTarefas:SENSITIVE = YES .
        END.
...
DO WITH FRAME fPage0
    ...
    IF tg-avalia-cq:CHECKED = YES THEN DO:
        ASSIGN {&DBOTempTable}.avalia_cq = YES .
    END.
    ELSE DO:
        ASSIGN {&DBOTempTable}.avalia_cq = NO .
    END.
    
    IF tg-armazena-especial:CHECKED = YES THEN DO:
        ASSIGN {&DBOTempTable}.armazena_especial = YES .
    END.
    ELSE DO:
        ASSIGN {&DBOTempTable}.armazena_especial = NO .
    END.
END.
...
RUN openQueryStatic IN hDBOProgram (INPUT "Recebimento":U) .
...
DO TRANSACTION ON ERROR UNDO , LEAVE ON STOP UNDO , LEAVE
    :
    FOR EACH wms_documento EXCLUSIVE-LOCK
        WHERE wms_documento.id_documento = {&DBOTempTable}.id_documento
        :
